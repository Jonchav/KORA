...
const FLUX_IMG2IMG_PROMPTS: Partial<Record<Style, string>> = {
  ...
  // ── Studio Session ─────────────────────────────────────────────────────────────
  studiowhite:
    "professional studio portrait photography. The entire background is completely replaced with a clean seamless white paper backdrop — no walls, no furniture, no original environment, only pure white or soft light gray studio backdrop behind the person. The person keeps their exact same face, body, hair, and clothing. Lighting: large octabox soft box key light at 45 degrees from the left, silver reflector opposite for fill, subtle hair rimlight, clean catch lights in eyes. Sharp focus on the subject, soft background. Corporate headshot quality — LinkedIn, executive profile, editorial. Shot on Canon EOS R5, 85mm f/1.8 lens, f/2.8 aperture. Clean, polished, professional. The background MUST be solid white studio paper — replace completely.",
  studiogray:
    "professional studio portrait photography. The entire background is completely replaced with a seamless medium gray gradient studio backdrop — no walls, no furniture, no original environment, only smooth gray paper behind the person. The person keeps their exact same face, body, hair, and clothing. Lighting: dramatic Rembrandt split lighting — strong key light at 45 degrees from above-left creating a triangle of light on the cheek, deep shadow on the opposite side, warm golden rim light from behind separating hair from background. Fashion editorial quality, moody, sophisticated. Shot on Sony A7R V, 85mm f/1.4 G Master, beautiful shallow bokeh on the gray background. The background MUST be seamless gray studio paper — replace completely.",
  studiodark:
    "professional studio portrait photography. The entire background is completely replaced with a white cyclorama wall with a single large circular spotlight projected onto it, creating a bold bright circle of light behind the person and a dramatic shadow cast to the side — exactly like high-fashion magazine studio photography. The person keeps their exact same face, body, hair, and clothing. Lighting: single strong side key light creating powerful contrast — bright highlights on one side, deep shadows on the other, crisp rim light separating the subject from the background, bright catch lights in eyes. High-contrast dramatic editorial style. Shot on Nikon Z9, 85mm f/1.2 lens. The background MUST be white wall with circular spotlight effect — replace completely.",
};
      ...
      const output = await replicate.run(
        "black-forest-labs/flux-dev" as `${string}/${string}`,
        {
          input: {
            image: dataUri,
            prompt,
            prompt_strength: MOVIE_SCENE_STYLES.has(style) ? 0.92 : STUDIO_STYLES.has(style) ? 0.80 : 0.60,
            num_inference_steps: MOVIE_SCENE_STYLES.has(style) ? 40 : STUDIO_STYLES.has(style) ? 40 : 35,
            guidance: MOVIE_SCENE_STYLES.has(style) ? 7.5 : STUDIO_STYLES.has(style) ? 5.0 : 3.5,
            output_format: "jpg",
            output_quality: 92,
          },
        }
...
async function runStudioPipeline(jobId: string, buf: Buffer, style: Style): Promise<Buffer> {
  ...
  // Step 1 — resize to working size (max 1280px on longest side)
  const meta = await sharp(buf).metadata();
  const srcW = meta.width ?? 1024;
  const srcH = meta.height ?? 1024;
  const maxDim = 1280;
